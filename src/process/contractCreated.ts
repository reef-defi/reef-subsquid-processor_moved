...
import { Account, Block, Contract, Extrinsic } from "../model";
...
export const saveContracts = async (contractsData: ContractData[], accounts: Map<string, Account>, extrinsics: Map<string, Extrinsic>, store: Storage): Promise<Map<string, Contract>> => {
    ...
    for (const contractData of contractsData) {
        ...
        if (!signer) {
            ...
            signer = await store.findBy(Account, {id: Equal(contractData.signerAddress)});
